...
public class EntitySandstormBody extends Mob implements IHurtableMultipart {

    private static final EntityDataAccessor<Integer> BODYINDEX = SynchedEntityData.defineId(EntitySandstormBody.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Float> BODY_XROT = SynchedEntityData.defineId(EntitySandstormBody.class, EntityDataSerializers.FLOAT);
    private static final EntityDataAccessor<Optional<UUID>> PARENT_UUID = SynchedEntityData.defineId(EntitySandstormBody.class, EntityDataSerializers.OPTIONAL_UUID);
    private static final EntityDataAccessor<Optional<UUID>> CHILD_UUID = SynchedEntityData.defineId(EntitySandstormBody.class, EntityDataSerializers.OPTIONAL_UUID);
    ...
    public EntitySandstormBody(EntityType t, LivingEntity parent, float radius, float angleYaw, float offsetY) {
    ...
    public void pushEntities() {
        ...
        if (parent != null) {
            entities.stream().filter(entity -> entity != parent && !(entity instanceof EntitySandstormBody) && entity.isPushable()).forEach(entity -> entity.push(parent));
        }
    }
    ...
    public boolean isFluidAt(double x, double y, double z) {
        if (this.noPhysics) {
            ...
            return !level().getFluidState(CSBlockPos.fromCoords(x, y, z)).isEmpty();
        }
    }
    ...
    public boolean isOpaqueBlockAt(double x, double y, double z) {
        if (this.noPhysics) {
            ...
            return this.level().getBlockStates(axisalignedbb).filter(Predicate.not(BlockBehaviour.BlockStateBase::isAir)).anyMatch((p_185969_) -> {
                final BlockPos blockpos = CSBlockPos.fromVec3(vec3);
