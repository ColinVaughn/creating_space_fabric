...
public class RenderSandstormBody extends MobRenderer<EntitySandstormBody, AdvancedEntityModel<EntitySandstormBody>> {
    private static final ResourceLocation TEXTURE = new ResourceLocation("alexsmobs:textures/entity/cave_centipede.png");

    public RenderSandstormBody(EntityRendererProvider.Context renderManagerIn) {
        super(renderManagerIn, new ModelSandstorm<>(1), 0.5F);
    }

    protected float getFlipDegrees(EntitySandstormBody centipede) {
        return 180.0F;
    }

    @Override
    protected void setupRotations(EntitySandstormBody entity, PoseStack stack, float pitchIn, float yawIn, float partialTickTime) {
    ...
    public ResourceLocation getTextureLocation(EntitySandstormBody entity) {
        return TEXTURE;
    }
}